...
export const useProvinceDataSource = () => {
  ...
  return new CustomStore({
    ...
    update: async (key: string, values: any) => {
      ...
      try {
        const response = await clientGateApi.Mst_Province_Update(key, values);
    ...
    insert: async (values: Partial<Province>) => {
      ...
      try {
        const response = await clientGateApi.Mst_Province_Create(values);
        if (response.isSuccess) {
          notify(m('DataSavedSuccessfully'), 'success');
          return values;
        } else {
    ...
    load(loadOptions: LoadOptions<Province>) {
      ...
      return clientGateApi.Mst_Province_Search(params)
        .then((data) => {
          if (data.isSuccess) {
            // cache the result
