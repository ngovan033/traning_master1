...
export const useDealerDataSource = () => {
    ...
    return new CustomStore({
        ...
        update: async (key: string, values: any) => {
            ...
            const resp = await clientGateApi.Mst_Dealer_Update(key, values);
            if (resp.isSuccess) {
        ...
        remove: async (code: any) => {
            ...
            try {
                const response = await clientGateApi.Mst_Dealer_Delete(code);
        ...
        load(loadOptions) {
            ...
            return clientGateApi.Mst_Dealer_Search(params)
                .then((data) => {
