...
import { dataViewAtom, isUpdateAtom, refecthAtom } from "../components/store";
...
export const CustomerInfo = ({
    ...
    queryFn: async () => {
      if (refecthData !== "") {
        ...
        if (response.isSuccess) {
          ...
          viewDebitPopupRef?.current.setDataFor2Grid(
            response?.Data.lst_Ser_CusDebit,
            response?.Data.lst_Ser_Payment
          );
          setLoad(false);
  ...
  const handleView = async () => {
    ...
    if (response.isSuccess && response?.Data) {
      ...
      // viewDebitPopupRef?.current.setdata1(response?.Data.lst_Ser_CusDebit);
      // viewDebitPopupRef?.current.setdata2(response?.Data.lst_Ser_Payment);
    } else {
      showError({
        message: t(response._strErrCode),
        _strErrCode: response._strErrCode,
