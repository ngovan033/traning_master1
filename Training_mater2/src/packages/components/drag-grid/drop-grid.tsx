...
export const DropGrid = forwardRef(
  (
    {
      customerHeight,
      toolbarItems,
    ...
    const onAdd = (e: any) => {
      const dataDrop = e.itemData;
      const isSame = dataDrop.Subject === data[e.toIndex - 1]?.Subject;
      if (isSame) {
        const dataCopy = [...data];
        dataCopy[e.toIndex - 1] = {
          ...dataCopy[e.toIndex - 1],
          Priority: dataDrop.Priority,
        };
        setData(dataCopy);
        toast.success("Ghép thành công");
      } else {
        toast.warning("Không đúng vị trí");
      }
    };

    return (
      <div className={"sub-grid-b"}>
        <DataGrid
          width={"100%"}
